...
class LoungeAccess extends Component {
  ...
  render() {
    ...
    var loungesPricing = loungesPoints.map(airport => {
      return (
        this.context.upsalesPricing["Lounge"][airport]["pricing"]["ADT"] *
          this.context.numADT +
        this.context.upsalesPricing["Lounge"][airport]["pricing"]["CNN"] *
          this.context.numCNN
      );
    });
    ...
    for (var i = 0; i < loungesPoints.length; i++) {
      overall.push({
        airport: loungesPoints[i],
        hourlyPrice: loungesPricing[i]
      });
    }
