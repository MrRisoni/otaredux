export const ADD_PASSENGER_MASTER = 'ADD_PASSENGER_MASTER';
export const FIRST_LOAD_MASTER = 'FIRST_LOAD_MASTER';
export const PASSENGER_ADDED_MASTER = 'PASSENGER_ADDED_MASTER';
export const REMOVE_PASSENGER_MASTER = 'REMOVE_PASSENGER_MASTER';
export const PASSENGER_REMOVED_MASTER = 'PASSENGER_REMOVED_MASTER';
export const CHANGE_PASSENGER_MASTER = 'CHANGE_PASSENGER_MASTER';
export const EDIT_NAME_PASSENGER_MASTER = 'EDIT_NAME_PASSENGER_MASTER';
export const EDITED_NAME_PASSENGER_MASTER = 'EDITED_NAME_PASSENGER_MASTER';
export const EDIT_CONTACT_PASSENGER_MASTER = 'EDIT_CONTACT_PASSENGER_MASTER';
export const PASSENGER_ARRAY_CHANGED = 'PASSENGER_ARRAY_CHANGED';
export const ADD_BAG = 'ADD_BAG';
export const REMOVE_BAG = 'REMOVE_BAG';
export const ADD_MEAL = 'ADD_MEAL';
export const UPSALES_CHANGED = 'UPSALES_CHANGED';
export const PURCHASE_INSURANCE = 'PURCHASE_INSURANCE';
export const ADD_FLEXIBLE_TICKET = 'ADD_FLEXIBLE_TICKET';
export const REMOVE_FLEXIBLE_TICKET = 'REMOVE_FLEXIBLE_TICKET';
export const ADD_BLUE_RIBBON = 'ADD_BLUE_RIBBON';
export const REMOVE_BLUE_RIBBON = 'REMOVE_BLUE_RIBBON';
export const FETCH_COUNTRIES_FINISHED = 'FETCH_COUNTRIES_FINISHED';
export const CHANGE_CURRENCY = 'CHANGE_CURRENCY';
export const CHANGE_CABIN = 'CHANGE_CABIN';
export const REMOVE_MEAL = 'REMOVE_MEAL';
export const PICK_SEAT_NO = 'PICK_SEAT_NO';
export const RESET_SEATS = 'RESET_SEATS';
export const PICK_PAX_AND_SEG_FOR_PRESEAT = 'PICK_PAX_AND_SEG_FOR_PRESEAT';
export const ADD_WEBCHECKIN = 'ADD_WEBCHECKIN';
export const REMOVE_WEBCHECKIN = 'REMOVE_WEBCHECKIN';
export const ADD_LOUNGE_ACCESS = 'ADD_LOUNGE_ACCESS';
export const ADD_PARK_DAYS = 'ADD_PARK_DAYS';
export const SUB_PARK_DAYS = 'SUB_PARK_DAYS';
export const ADD_FAST_TRACK = 'ADD_FAST_TRACK';
export const REMOVE_FAST_TRACK = 'REMOVE_FAST_TRACK';
